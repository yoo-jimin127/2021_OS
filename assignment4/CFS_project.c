	...
	for (int i = 0; i < 21; i++) {
		...
		if (pids[i] == -1) { // fork() error
			...
			if (i >= 0 && i <= 6) { // process group A
				...
				if (result == -1) {
					perror("ERROR: error occurs at calling sched_setattr <group A>.\n");
				}
				
				for (long long a = 0; a < 60000; a++) {
					for (long long b = 0; b < 5000; b++) {
						num = a * b;
					}
				}
			}
